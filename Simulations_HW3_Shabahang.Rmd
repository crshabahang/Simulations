...
# QFGB 8925-002
# The program evaluates the returns of a rolling protective put strategy 
# Reference: Figlewski, Stephen, Chidambaran, N.K and Kaplan, Scott. Evaluating the performance of the protective put strategy. 
# Financial Analysts Journal, Jul/Aug 1993, 49, 4, p. 46
```{r}
...
MYBSPut<-function(S0,k0,rf,m0,Volatility) {
    d1 <- (log(S0/k0) + (rf + 0.5*Volatility^2)*tau)/(Volatility*sqrt(m0))
    d2 <- d1 - Volatility*sqrt(tau)
    bs_put <- k0*exp(-rf*(m0))*pnorm(-d2)- S0*pnorm(-d1)
    return (bs_put)
}
# Normal random number generator
# Inputs: seed and numran
...
hist(rand_norm_c,xlim=c(-5,5),breaks=20,freq=F) #Generates a histogram of the normally distributed random numbers
curve(dnorm(x, mean=0, sd=1),from=-4, to=4,add=TRUE,lwd=2) #Generates a normal curve to overlay on the histogram of generated numbers

#Protective put price paths
...
for (scount in 1:2) { #For loops allow for optionality in values of strike, drift, and volatility parameters 
...
T<-1/12
numpath<-10000
PPut_Rets<-rep(0,numpath) #The rep() function replicates values to initialize the put returns and stock returns matrices
...
# Initiate the rollover process 
  ...
  m0<-2/12
  PPut_Val<--(MYBSPut(S0,k0,rf,m0,Volatility)) ###Need to input a B-S put function 
# Rollover over monthly prices for 1-year
    for (kcount in 1:12) {
    S_t1<-S_t*(exp(drift*T+Volatility*sqrt(T)*rand_norm_c[lcount])) #GBM stock price process 
    ...
    if (kcount > 11) {
      PPut_Val <- PPut_Val*exp(rf/12)+PP_t+S_t1 #Protective put value is discounted and added to 1 month option at old strike and stock price
      PPut_Rets[jcount]<-log(PPut_Val/S0)
      St_Ret[jcount]<-log(S_t1/S0)
      if(PPut_Val < 0) {cat("1",PPut_Val,S_t1,mt1,kt,kt1,PP_t,PP_t1,"\n")} #If the protective put value is negative at last month, display output
      }
    if (kcount < 12) {
      PPut_Val <- PPut_Val*exp(+rf*1/12)+(PP_t-PP_t1) #For all other months, discount by one month and add difference in put option prices
      }
...
# Store results for the stock price process 
...
#Evaluate alternate strike price strategies, other than a constant proportion of stock price 
```
