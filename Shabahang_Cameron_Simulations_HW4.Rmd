...
payoff_expiry_put_a <-pmax(K-S_analytic,0)
expected_payoff_put_a <- sum(payoff_expiry_put_a)/length(payoff_expiry_put_a)
Monte_Carlo_put_price_a <- exp(-r*(tau))*expected_payoff_put_a

#American put valuation via analytic 
payoff_expiry_put_Am_a <-pmax(K-S_analytic,0)
for(k in 1:nSim){
  boundary = 0.2*S_analytic[k]
...
cat('\nThe Monte Carlo American analytic put price is: ', Monte_Carlo_Am_put_price_a, '\n')
```
```{r}
###Part II
